...
class Journal(AnalyticMixin):
    __name__ = 'account.journal'

    analytics = fields.Boolean('Analytics On Balance Accounts')
    analytic_accounts = fields.Many2One(
        'analytic_account.account.selection', 'Analytic Accounts', states={
            'required': Bool(Eval('analytics')),
            'invisible': ~Bool(Eval('analytics')),
            }, depends=['analytics'])

    def set_analytic_lines(self, move):
        """
        Sets analytics lines for a move based on analytic account in journal
        """
        to_create = []
        for line in move.lines:
            analytic_lines = self.get_analytic_lines(move, line)
            if analytic_lines:
                to_create += analytic_lines
        return to_create
    ...
    def delete_analytic_lines(self, move):
        """
        Delete analytic lines of move
        """
        pool = Pool()
        AnalyticLine = pool.get('analytic_account.line')
        for line in move.lines:
            AnalyticLine.delete(AnalyticLine.search([
                    ('id', 'in', [l.id for l in line.analytic_lines]),
                    ('from_journal', '=', True),
                    ]))
...
class Account:
    ...
    def delete(cls, accounts):
        ...
        Journal._fields_view_get_cache.clear()
